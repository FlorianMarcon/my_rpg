...
void	dep_right(game_t *game)
{
	...
	if (game->perso->x != 0 && detect_col(game->perso, game->map, 1) != -1) {
		while (game->perso->position.x < x + 26.4 &&
					game->perso->position.y < y + 18.2) {
			loop_move(my_clock, game->perso, game->win);
			display(game, NULL, NULL, NULL);
		}
		game->perso->x -= 1;
		game->perso->position.x = game->map->iso[game->perso->x][game->perso->y].x - 27;
		game->perso->position.y = game->map->iso[game->perso->x][game->perso->y].y - 32;
...
void	dep_up(game_t *game)
{
	...
	if (game->perso->y != 0 && detect_col(game->perso, game->map, 11) != -1) {
		game->perso->mouvement.y = 1;
		game->perso->mouvement.x = 0;
		while (game->perso->position.x > x - 36.4 && game->perso->position.y > y - 13.4) {
			loop_move(my_clock, game->perso, game->win);
